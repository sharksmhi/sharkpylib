...
8043		VTDH		m		sea_surface_wave_significant_height				SPECTRAL_SIGNIFICANT_WAVE_HEIGHT_(HM0)
...
11	THRSHLOW	threshold low value for the spike test [unit/min]
12	THRSHHIGH	threshold high value for the spike test, THRSHLD_HIGH=abs[y(n-1)-(y(n-2)+y(0))/2] [unit/min]
13	RCNTF		repeated observations (X minutes) for the flat line test, REP_CNT_FAIL=X
14	RCNTS		repeated observations (X minutes) for the flat line test, REP_CNT_SUSPECT=X
15	TEPS		tolerance value for the flat line test to allow for a numerical round-off error, EPS [m]
16	NDEV		Number of standard deviation to use in rate of change test, alt1.
17	TIMDEV		Time to compute standard deviation for rate of change test, alt1. Minute
(18	DEV		Rate of change to use in rate of change test, alt2. [unit/min])
19	NOBS		number of observations
20	CELL		grid cell number (use only for station type FB=Ferrybox)
